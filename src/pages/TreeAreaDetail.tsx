
import React from 'react';
import { useParams } from 'react-router-dom';
import TreeHeader from '../components/TreeHeader';
import EnhancedMapboxMap from '../components/EnhancedMapboxMap';
import Footer from '../components/Footer';
import { MapPin, Phone, Clock, Star, CheckCircle } from 'lucide-react';

const TreeAreaDetail = () => {
  const { slug } = useParams();
  
  const areas = {
    'downtown-district': {
      name: 'Downtown District',
      coordinates: { lat: 40.7128, lng: -74.0060 },
      description: 'Professional tree care services in the heart of Downtown District',
      features: ['Emergency Tree Removal', '24/7 Service', 'Certified Arborists', 'Insurance Claims'],
      neighborhoods: ['Financial District', 'City Center', 'Business District', 'Historic Quarter']
    },
    'greenwood-heights': {
      name: 'Greenwood Heights',
      coordinates: { lat: 40.7589, lng: -73.9441 },
      description: 'Expert tree removal and care services for Greenwood Heights residents',
      features: ['Residential Tree Care', 'Storm Damage Cleanup', 'Tree Health Assessment', 'Stump Grinding'],
      neighborhoods: ['Greenwood Park', 'Heights Manor', 'Oak Hills', 'Maple Estates']
    },
    'oak-valley': {
      name: 'Oak Valley',
      coordinates: { lat: 40.6782, lng: -73.9442 },
      description: 'Complete tree services for Oak Valley and surrounding areas',
      features: ['Tree Trimming', 'Disease Treatment', 'Emergency Response', 'Land Clearing'],
      neighborhoods: ['Oak Valley Center', 'Valley View', 'Oak Grove', 'Valley Heights']
    },
    'pine-ridge': {
      name: 'Pine Ridge',
      coordinates: { lat: 40.8176, lng: -73.9482 },
      description: 'Trusted tree care professionals serving Pine Ridge community',
      features: ['Pine Tree Specialists', 'Hazardous Tree Removal', 'Property Maintenance', 'Free Estimates'],
      neighborhoods: ['Pine Ridge Estates', 'Ridge View', 'Pine Grove', 'Mountain View']
    },
    'maple-grove': {
      name: 'Maple Grove',
      coordinates: { lat: 40.7282, lng: -73.7949 },
      description: 'Professional tree services for Maple Grove residential and commercial properties',
      features: ['Maple Tree Care', 'Commercial Services', 'Tree Preservation', 'Crane Service'],
      neighborhoods: ['Maple Grove Village', 'Grove Park', 'Maple Heights', 'Grove Manor']
    }
  };

  const area = areas[slug as keyof typeof areas];

  if (!area) {
    return <div>Area not found</div>;
  }

  return (
    <div className="min-h-screen font-poppins bg-gradient-to-br from-forest-50 to-nature-50">
      <TreeHeader />
      
      {/* Hero Section */}
      <section className="relative py-20 bg-gradient-to-r from-forest-600 to-nature-600 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-4xl md:text-6xl font-bold mb-6">
              Tree Care Services in
              <span className="block text-nature-300">{area.name}</span>
            </h1>
            <p className="text-xl md:text-2xl mb-8 max-w-3xl mx-auto">
              {area.description}
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a 
                href="tel:5551234567"
                className="bg-gradient-to-r from-nature-500 to-nature-600 hover:from-nature-400 hover:to-nature-500 text-black px-8 py-4 rounded-full font-bold text-lg transition-all duration-300 flex items-center justify-center transform hover:scale-105"
              >
                <Phone className="mr-2" size={20} />
                Call Now: (555) 123-4567
              </a>
            </div>
          </div>
        </div>
      </section>

      {/* Map Section */}
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              Service Area Coverage
            </h2>
            <p className="text-xl text-gray-600">
              Interactive map showing our tree care service coverage in {area.name}
            </p>
          </div>
          
          <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
            <EnhancedMapboxMap className="h-96 md:h-[500px]" />
          </div>
        </div>
      </section>

      {/* Service Features */}
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              Tree Services in {area.name}
            </h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {area.features.map((feature, index) => (
              <div key={index} className="text-center p-6 bg-gradient-to-br from-forest-50 to-nature-50 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300">
                <div className="w-16 h-16 bg-gradient-to-r from-forest-500 to-nature-500 rounded-full flex items-center justify-center mx-auto mb-4">
                  <CheckCircle size={32} className="text-white" />
                </div>
                <h3 className="text-lg font-bold text-gray-900 mb-2">{feature}</h3>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* Neighborhoods */}
      <section className="py-16 bg-gradient-to-br from-forest-50 to-nature-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              Neighborhoods We Serve
            </h2>
            <p className="text-xl text-gray-600">
              Professional tree care throughout {area.name}
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {area.neighborhoods.map((neighborhood, index) => (
              <div key={index} className="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 text-center">
                <div className="w-12 h-12 bg-gradient-to-r from-forest-500 to-nature-500 rounded-full flex items-center justify-center mx-auto mb-4">
                  <MapPin size={24} className="text-white" />
                </div>
                <h3 className="text-lg font-bold text-gray-900">{neighborhood}</h3>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* CTA Section */}
      <section className="py-16 bg-gradient-to-r from-forest-600 to-nature-600 text-white">
        <div className="max-w-4xl mx-auto text-center px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl md:text-4xl font-bold mb-6">
            Need Tree Service in {area.name}?
          </h2>
          <p className="text-xl mb-8">
            Call our certified arborists for professional tree care services. Same-day emergency service available!
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            <a 
              href="tel:5551234567"
              className="bg-gradient-to-r from-nature-500 to-nature-600 hover:from-nature-400 hover:to-nature-500 text-black px-8 py-4 rounded-full font-bold text-lg transition-all duration-300 flex items-center transform hover:scale-105"
            >
              <Phone className="mr-2" size={20} />
              Call Now: (555) 123-4567
            </a>
            <div className="flex items-center text-lg">
              <Clock className="mr-2" size={20} />
              24/7 Emergency Service
            </div>
          </div>
        </div>
      </section>

      <Footer />
    </div>
  );
};

export default TreeAreaDetail;
